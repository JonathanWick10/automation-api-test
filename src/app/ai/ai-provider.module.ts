...
const { AI_USAGE_AGENT_ID_HEADER, AI_USAGE_FEATURE_HEADER, AI_USAGE_MCP_ID_HEADER, AIUsageFeature, SUPPORTED_AI_PROVIDERS } = commonai;
import { exceptionHandler } from 'automation-server-shared-test'
import { ActivepiecesError, ErrorCode, isNil, PlatformUsageMetric, PrincipalType } from 'automation-shared-test'
...
function getProviderConfigOrThrow(provider: string | undefined): commonai.SupportedAIProvider {
...
function buildAIUsageMetadata(headers: Record<string, string | string[] | undefined>): commonai.AIUsageMetadata {
