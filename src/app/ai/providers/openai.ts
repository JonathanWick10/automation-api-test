import { ActivepiecesError, ErrorCode,  isNil } from 'automation-shared-test'
...
export const openaiProvider: AIProviderStrategy = {
    ...
    calculateUsage: (request: FastifyRequest<RequestGenericInterface, RawServerBase>, response: Record<string, unknown>): Usage => {
        ...
        if (languageModelConfig) {
            ...
            const { input: inputCost, output: outputCost } = languageModelConfig.pricing as commonai.FlatLanguageModelPricing
        ...
        if (imageModelConfig?.instance.modelId === 'dall-e-3') {
            const pricing = imageModelConfig.pricing as commonai.DALLE3PricingPerImage
        ...
        if (imageModelConfig?.instance.modelId === 'gpt-image-1') {
            const pricing = imageModelConfig.pricing as commonai.GPTImage1PricingPerImage
        ...
        const pricing = imageModelConfig?.pricing as commonai.DALLE2PricingPerImage
    ...
    getAuthHeaders: (config: commonai.AIProvider['config']): Record<string, string> => {
    ...
    getBaseUrl: (config: commonai.AIProvider['config']): string => {
        if (config.azureOpenAI) {
    ...
    rewriteUrl: (config: commonai.AIProvider['config'], originalUrl: string): string => {
        if (config.azureOpenAI) {
