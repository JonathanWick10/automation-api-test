...
export const aiProviderService = {
    async list(userPlatformId: PlatformId): Promise<SeekPage<commonai.AIProviderWithoutSensitiveData>> {
        const platformId = await this.getAIProviderPlatformId(userPlatformId)

        const providers = await aiProviderRepo().findBy({ platformId })

        const aiProviders = providers.map((provider): commonai.AIProviderWithoutSensitiveData => ({
            id: provider.id,
    ...
    async upsert(platformId: PlatformId, request: commonai.CreateAIProviderRequest): Promise<void> {
    ...
    async getConfig(provider: string, platformId: PlatformId): Promise<commonai.AIProvider['config']> {
    ...
    getBaseUrl(provider: string, config: commonai.AIProvider['config']): string {
    ...
    rewriteUrl(provider: string, config: commonai.AIProvider['config'], originalUrl: string): string {
        const providerStrategy = aiProvidersStrategies[provider]
    ...
    getAuthHeaders(provider: string, config: commonai.AIProvider['config']): Record<string, string> {
        const providerStrategy = aiProvidersStrategies[provider]
...
function getProviderConfig(provider: string | undefined): commonai.SupportedAIProvider | undefined {
    if (isNil(provider)) {
        return undefined
    }
...
function validateAIUsageHeaders(headers: Record<string, string | string[] | undefined>): void {
    const feature = headers[AI_USAGE_FEATURE_HEADER] as commonai.AIUsageFeature
    ...
    const supportedFeatures = Object.values(AIUsageFeature).filter(f => f !== AIUsageFeature.UNKNOWN) as commonai.AIUsageFeature[]
